...
const Category = async() => {
  ...
  return (
    <div className='pt-16 pb-12'>
        ...
        {/* Define Grid */}
        <div className='mt-12 w-4/5 mx-auto grid grid-cols-2 lg:grid-cols-4 gap-8'>
            {categories.map((category) =>{
                return (
                    <div
                        key={category}
                        className='p-6 rounded-lg cursor-pointer text-center hover:scale-110 transition-all
                        duration-300 bg-gray-200 shadow-md'
                    >
                            <h1 className='text-sm sm:text-base md:text-[20px] capitalize font-bold'>
                                {category}
                            </h1>
                    </div>
                )
